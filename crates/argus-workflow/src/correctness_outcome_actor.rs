...
use crate::{
    CorrectnessAssessmentContract, EffectiveOutcome, LogicalOutcomeKey, OutcomeDisposition,
...
use argus_policies::{CorrectnessAssessment, CorrectnessResult};
...
/// Stores one validated correctness assessment before committing its durable outcome reference.
pub struct CorrectnessOutcomeActor {
...
pub struct DurableCorrectnessOutcomeActor {
    inbox: Arc<DurableQueue>,
    workflow_data: Arc<WorkflowDataStore>,
    contract: Arc<CorrectnessAssessmentContract>,
...
impl DurableCorrectnessOutcomeActor {
    ...
    async fn record_for_run(&self, langchart_run_id: &str) -> Result<OutcomeReceipt, String> {
        ...
        CorrectnessOutcomeActor::from_decision(
            self.inbox.clone(),
...
impl CorrectnessOutcomeActor {
    pub fn from_decision(
        inbox: Arc<DurableQueue>,
        contract: &CorrectnessAssessmentContract,
    ...
    pub fn new(
            ...
            || assessment.policy_version != logical_key.policy_version
        {
            return Err(argus_core::ArgusError::invariant(
                "correctness assessment identity does not match its outcome key",
            ));
        }
        Ok(Self {
            inbox,
    ...
    pub fn record(&self) -> Result<OutcomeReceipt, String> {
        ...
        let stored = self
            .inbox
            .store_artifact(CORRECTNESS_ASSESSMENT_ARTIFACT_KIND, &assessment_bytes)
            .map_err(|error| error.to_string())?;
        let effective_outcome = EffectiveOutcome {
            ...
            kind: match self.assessment.result {
                CorrectnessResult::Passed => OutcomeKind::Passed,
                CorrectnessResult::CandidateFindings { .. } => OutcomeKind::CandidateFindings,
                CorrectnessResult::UnableToVerify { .. } => OutcomeKind::UnableToVerify,
            },
...
#[async_trait]
impl AgentActor for DurableCorrectnessOutcomeActor {
    async fn run(
        &self,
