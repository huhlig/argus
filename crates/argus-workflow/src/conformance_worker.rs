...
#[derive(Clone, Debug)]
pub struct ConformanceWorkerConfig {
    pub state_directory: PathBuf,
    pub identity: ConformanceRuntimeIdentity,
    pub adapter: String,
    pub policy: String,
...
pub struct ConformanceWorker {
    queue: Arc<DurableQueue>,
    workflow_data: Arc<WorkflowDataStore>,
    runtime: DocumentationWorkerRuntime,
    config: ConformanceWorkerConfig,
    checkpoint_store: Arc<dyn CheckpointStore>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum ConformanceWorkerResult {
    Idle,
...
impl ConformanceWorker {
    pub fn new(
        ...
        if config.lease_duration_millis == 0 || config.maximum_attempts == 0 {
            return Err(ArgusError::invalid_input(
                "conformance worker lease and attempt limits must be positive",
            ));
        }
        if config.adapter.is_empty() || config.policy.is_empty() {
            return Err(ArgusError::invalid_input(
                "conformance worker adapter and policy must not be empty",
            ));
        }
        let checkpoint_store = Arc::new(open_checkpoint_store(&config.state_directory).map_err(
            |error| {
                ArgusError::invariant("cannot open conformance checkpoint store").with_source(error)
            },
        )?);
        Ok(Self {
            queue,
    ...
    pub async fn run_next(&self, now_millis: u64) -> Result<ConformanceWorkerResult, ArgusError> {
        let leased = {
            ...
            .await
            .map_err(|error| {
                ArgusError::invariant("conformance lease task failed").with_source(error)
            })??
        };
        let Some(leased) = leased else {
            return Ok(ConformanceWorkerResult::Idle);
        };
            ...
            Err(error) => {
                ...
                let succeeded_already = {
                    ...
                    tokio::task::spawn_blocking(move || queue.get(&work_id))
                        .await
                        .map_err(|error| {
                            ArgusError::invariant("conformance lookup task failed")
                                .with_source(error)
                        })??
                };
                if succeeded_already.is_some_and(|work| work.state == QueueState::Succeeded) {
                    return Ok(ConformanceWorkerResult::Succeeded { work_id: leased.id });
                }
                let queue = self.queue.clone();
                let work_id = leased.id.clone();
                ...
                .await
                .map_err(|error| {
                    ArgusError::invariant("conformance fail-attempt task failed").with_source(error)
                })??;
                Ok(match state {
                    QueueState::Pending => ConformanceWorkerResult::RetryScheduled {
                        work_id: leased.id,
                        error: message,
                    },
                    QueueState::Failed => ConformanceWorkerResult::Failed {
                        work_id: leased.id,
                        error: message,
                    },
                    _ => {
                        return Err(ArgusError::invariant(
                            "failed conformance attempt entered an invalid queue state",
                        ));
                    }
                })
            }
    ...
    async fn execute_with_heartbeats(
        ...
        let mut heartbeat_task = tokio::spawn(async move {
            ...
            loop {
                ...
                .await
                .map_err(|error| {
                    ArgusError::invariant("conformance heartbeat task failed").with_source(error)
                })??;
            }
        ...
        let result = tokio::select! {
            ...
            heartbeat = &mut heartbeat_task => heartbeat
                .map_err(|error| ArgusError::invariant("conformance heartbeat task failed").with_source(error))?,
        };
    ...
    async fn execute(&self, leased: &LeasedWork) -> Result<(), ArgusError> {
        ...
        tracing::info!(
            ...
            "Processing conformance review for {:?} target `{}`",
        ...
        let checkpoint = prepared
            ...
            .await
            .map_err(|error| {
                ArgusError::invariant("cannot load conformance checkpoint").with_source(error)
            })?;
        let resolver = Arc::new(ConformanceContextResolver {
            ...
            content: String::from_utf8(materialized.context.canonical_json.clone()).map_err(
                |error| {
                    ArgusError::invariant("conformance context is not UTF-8").with_source(error)
                },
            )?,
        ...
        if let Some(snapshot) = checkpoint {
            let checkpoint: InstanceCheckpoint = serde_json::from_slice(&snapshot.payload)
                .map_err(|error| {
                    ArgusError::invalid_input("invalid conformance workflow checkpoint")
                        .with_source(error)
                })?;
            match checkpoint.status {
                RunStatus::Suspended => {
                    ...
                    instance.resume().await.map_err(|error| {
                        ArgusError::invariant("cannot resume conformance workflow")
                            .with_source(error)
                    })?;
                }
                RunStatus::Completed => {
                ...
                RunStatus::Failed | RunStatus::Cancelled | RunStatus::Running => {
                    return Err(ArgusError::invariant(
                        "conformance checkpoint is not safely resumable",
                    ));
                }
            }
        } else {
            instance.start().await.map_err(|error| {
                ArgusError::invariant("cannot start conformance workflow").with_source(error)
            })?;
        }
        let status = instance.run_to_completion().await.map_err(|error| {
            ArgusError::invariant("conformance workflow execution failed").with_source(error)
        })?;
        if status != RunStatus::Completed {
            ...
            return Err(ArgusError::invariant(format!(
                "conformance workflow ended with {status:?}{detail}"
            )));
        }
    ...
    fn restore_work(
        ...
        (
            ConformanceReviewAdmission,
            ConformanceReviewMaterialization,
            RunId,
        ),
        ArgusError,
    > {
        let admission: ConformanceReviewAdmission = serde_json::from_slice(&leased.payload)
            .map_err(|error| {
                ArgusError::invalid_input("invalid conformance review admission").with_source(error)
            })?;
        if admission.unit.work_item != leased.id {
            return Err(ArgusError::invariant(
                "leased work does not match its conformance admission",
            ));
        }
        let owned = self
            .queue
            .get(&leased.id)?
            .ok_or_else(|| ArgusError::invariant("leased conformance work is missing"))?;
        if owned.run != self.config.identity.audit_run
            || owned.coverage.snapshot != self.config.identity.audit_snapshot.as_str()
        {
            return Err(ArgusError::invariant(
                "conformance worker identity does not own the leased work",
            ));
        }
        let materialized = ConformanceReviewMaterialization::restore(&self.queue, &admission)?;
        if materialized.package.package.snapshot != self.config.identity.audit_snapshot {
            return Err(ArgusError::invariant(
                "conformance package is outside the worker snapshot",
            ));
        }
        ...
        let retry_generation = u64::try_from(retry_generation)
            .map_err(|_| ArgusError::invariant("conformance retry generation overflow"))?;
        ...
        materialized
            ...
            .map_err(|error| {
                ArgusError::invariant("cannot initialize conformance workflow data")
                    .with_source(error)
            })?;
    ...
    fn prepare_runtime(
        &self,
        leased: &LeasedWork,
        materialized: &ConformanceReviewMaterialization,
        langchart_run_id: &RunId,
    ) -> Result<PreparedConformanceRuntime, ArgusError> {
        let recovery = RecoveryStore::open(&self.config.state_directory).map_err(|error| {
            ArgusError::invariant("cannot open conformance recovery store").with_source(error)
        })?;
        let workflow = recovery.store_target_review().map_err(|error| {
            ArgusError::invariant("cannot store conformance workflow").with_source(error)
        })?;
        let manifest = match recovery.load_manifest(langchart_run_id.as_ref()) {
            Ok(mut existing) => {
                ...
                existing.actors =
                    recovery
                        ...
                        .map_err(|error| {
                            ArgusError::invariant("cannot resolve conformance actor identities")
                                .with_source(error)
                        })?;
                existing
            }
            Err(RecoveryError::Io(error)) if error.kind() == std::io::ErrorKind::NotFound => {
                ...
                recovery.write_manifest(&manifest).map_err(|error| {
                    ArgusError::invariant("cannot store conformance recovery manifest")
                        .with_source(error)
                })?;
                manifest
            }
            Err(error) => {
                return Err(
                    ArgusError::invariant("cannot load conformance recovery manifest")
                        .with_source(error),
                );
            }
        };
        let compiled = Arc::new(
            recovery
                ...
                .map_err(|error| {
                    ArgusError::invariant("cannot load conformance workflow").with_source(error)
                })?,
        );
        let registry = conformance_actor_registry(
            self.queue.clone(),
        ...
        .map_err(|error| {
            ArgusError::invariant("cannot assemble conformance actor registry").with_source(error)
        })?;
        let actors = registry.reconstruct(&manifest).map_err(|error| {
            ArgusError::invariant("cannot reconstruct conformance actors").with_source(error)
        })?;
        Ok(PreparedConformanceRuntime {
            compiled,
            actors,
    ...
    fn require_durable_outcome(&self, leased: &LeasedWork) -> Result<(), ArgusError> {
        let work = self
            .queue
            .get(&leased.id)?
            .ok_or_else(|| ArgusError::invariant("conformance work disappeared"))?;
        if work.state != QueueState::Succeeded {
            return Err(ArgusError::invariant(
                "conformance workflow completed without a durable outcome",
            ));
        }
        Ok(())
    }
    ...
    fn require_durable_result(
        ...
        let record = self
            ...
            .load(langchart_run_id)
            .map_err(|error| {
                ArgusError::invariant("cannot load terminal conformance decision")
                    .with_source(error)
            })?
            .ok_or_else(|| ArgusError::invariant("terminal conformance decision is missing"))?;
            ...
            && decision.event_type == "review.failed"
        {
            ...
            return Err(ArgusError::invariant(format!(
                "conformance review declared failure: {reason}"
            )));
        }
...
fn langchart_run_id(audit_run: &AuditRunId, work_id: &WorkItemId, retry_generation: u64) -> RunId {
    ...
    hasher.update(b"argus.conformance.langchart-run.v1\0");
    ...
    RunId::new(format!("argus-conformance-{}", hasher.finalize().to_hex()))
}

struct ConformanceContextResolver {
    run_id: RunId,
    source: String,
    content: String,
    tokens: u32,
...
#[async_trait]
impl ContextResolver for ConformanceContextResolver {
    async fn resolve(
        &self,
