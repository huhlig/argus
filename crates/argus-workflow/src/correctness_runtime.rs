...
use crate::{
    ActorRegistry, ActorRegistryError, CandidateRecorderActor, CorrectnessReviewMaterialization,
    DurableCorrectnessOutcomeActor, EvidenceRequestEvaluatorActor, FindingWorkSchedulerActor,
...
pub fn correctness_actor_registry(
    ...
    identity: CorrectnessRuntimeIdentity,
) -> Result<ActorRegistry, ActorRegistryError> {
    let mut registry = ActorRegistry::new();
    register(
        &mut registry,
        "argus.prepare-evidence",
        Arc::new(PrepareCorrectnessEvidenceActor::new(
            workflow_data.clone(),
            materialized.clone(),
        )),
    )?;
    ...
    register(
        ...
        Arc::new(DurableCorrectnessOutcomeActor::new(
            queue,
            workflow_data,
...
struct PrepareCorrectnessEvidenceActor {
    workflow_data: Arc<WorkflowDataStore>,
    materialized: CorrectnessReviewMaterialization,
}

impl PrepareCorrectnessEvidenceActor {
    const fn new(
        workflow_data: Arc<WorkflowDataStore>,
        materialized: CorrectnessReviewMaterialization,
    ) -> Self {
...
#[async_trait]
impl AgentActor for PrepareCorrectnessEvidenceActor {
    async fn run(
        &self,
...
impl AgentActor for DisabledEvidenceExpansionActor {
    async fn run(
        ...
        Err(AgentError::Internal(
            "correctness evidence expansion is disabled for this admission".to_owned(),
        ))
    }
}
