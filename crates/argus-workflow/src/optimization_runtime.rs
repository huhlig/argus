...
use crate::{
    ActorRegistry, ActorRegistryError, CandidateRecorderActor, DurableOptimizationOutcomeActor,
    EvidenceRequestEvaluatorActor, FindingWorkSchedulerActor, OptimizationReviewMaterialization,
...
/// Runtime identity and configuration parameters for optimization review execution.
#[derive(Clone, Debug)]
pub struct OptimizationRuntimeIdentity {
...
/// Builds and populates an [`ActorRegistry`] with the actors required for optimization review.
///
/// Registers actors for the standard optimization review pipeline:
/// - `argus.prepare-evidence`: Prepares optimization evidence from materialized targets.
...
pub fn optimization_actor_registry(
    ...
    register(
        ...
        Arc::new(DurableOptimizationOutcomeActor::new(
            queue,
            workflow_data,
...
struct PrepareOptimizationEvidenceActor {
    workflow_data: Arc<WorkflowDataStore>,
    materialized: OptimizationReviewMaterialization,
}

impl PrepareOptimizationEvidenceActor {
    const fn new(
        workflow_data: Arc<WorkflowDataStore>,
        materialized: OptimizationReviewMaterialization,
    ) -> Self {
...
#[async_trait]
impl AgentActor for PrepareOptimizationEvidenceActor {
    async fn run(
        &self,
        ...
        tracing::debug!(
            actor = "PrepareOptimizationEvidenceActor",
            ...
            "Entering workflow state: PrepareOptimizationEvidenceActor"
        );
            ...
            || record.data.evidence_revision != self.materialized.package.package.revision
        {
            return Err(AgentError::Internal(
                "prepared optimization evidence identity mismatch".to_owned(),
            ));
        }
        tracing::debug!(
            actor = "PrepareOptimizationEvidenceActor",
            "Exiting workflow state: PrepareOptimizationEvidenceActor -> evidence.prepared"
        );
        Ok(AgentOutputEvent {
...
impl AgentActor for DisabledEvidenceExpansionActor {
    async fn run(
        ...
        Err(AgentError::Internal(
            "optimization evidence expansion is disabled for this admission".to_owned(),
        ))
    }
}
