...
use argus_policies::{
    OptimizationApplicabilityDecision, OptimizationApplicabilityPolicy, OptimizationTargetClass,
...
pub const OPTIMIZATION_REVIEW_PLAN_SCHEMA_VERSION: u32 = 1;
pub const OPTIMIZATION_EVIDENCE_PACKAGE_ARTIFACT_KIND: &str = "evidence-package";
pub const OPTIMIZATION_REVIEW_CONTEXT_ARTIFACT_KIND: &str = "review-context";

#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct OptimizationReviewUnit {
    pub schema_version: u32,
    pub work_item: WorkItemId,
    pub target: OptimizationTargetProfile,
    pub policy: PolicyId,
    pub policy_version: String,
    pub applicability: OptimizationApplicabilityDecision,
    pub evidence: Vec<EvidenceId>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct OptimizationReviewPlan {
    pub units: Vec<OptimizationReviewUnit>,
}

#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct OptimizationReviewAdmission {
    pub schema_version: u32,
    pub unit: OptimizationReviewUnit,
    pub evidence_package_ref: String,
    pub review_context_ref: String,
}

#[derive(Clone, Debug)]
pub struct OptimizationReviewBatch {
    pub materializations: Vec<OptimizationReviewMaterialization>,
}

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct OptimizationEvidenceCatalog {
    hashes: BTreeMap<EvidenceId, ContentHash>,
}

impl OptimizationEvidenceCatalog {
    pub fn ingest(
        store: &EvidenceStore,
        snapshot: &SnapshotId,
        ...
        for record in records {
            if hashes.contains_key(&record.id) {
                return Err(argus_core::ArgusError::invariant(
                    "optimization evidence catalog contains duplicate IDs",
                ));
            }
            let envelope =
...
#[derive(Clone, Debug)]
pub struct OptimizationReviewMaterialization {
    pub unit: OptimizationReviewUnit,
    pub package: PackageArtifact,
    pub context: ContextArtifact,
    pub contract: Arc<crate::OptimizationAssessmentContract>,
}

impl OptimizationReviewMaterialization {
    pub fn restore(
        queue: &DurableQueue,
        admission: &OptimizationReviewAdmission,
    ) -> Result<Self, argus_core::ArgusError> {
        if admission.schema_version != OPTIMIZATION_REVIEW_PLAN_SCHEMA_VERSION
            || admission.unit.schema_version != OPTIMIZATION_REVIEW_PLAN_SCHEMA_VERSION
            || admission.unit.applicability.state != ApplicabilityState::Applicable
        {
            return Err(argus_core::ArgusError::unsupported(
                "unsupported or inapplicable optimization review admission",
            ));
        }
        ...
        let stored_context = load_artifact(
            ...
            OPTIMIZATION_REVIEW_CONTEXT_ARTIFACT_KIND,
        )?;
        let frame: ReviewContextFrame =
            serde_json::from_slice(&stored_context.payload).map_err(|error| {
                argus_core::ArgusError::invalid_input("invalid stored optimization review context")
                    .with_source(error)
            })?;
        ...
        let contract = Arc::new(crate::OptimizationAssessmentContract::from_context(
...
impl OptimizationReviewUnit {
    pub fn materialize(
        &self,
        store: &EvidenceStore,
        catalog: &OptimizationEvidenceCatalog,
        snapshot: &SnapshotId,
        configuration: &ConfigurationId,
        budget: EvidenceBudget,
        maximum_classification: DataClassification,
    ) -> Result<OptimizationReviewMaterialization, argus_core::ArgusError> {
        if self.schema_version != OPTIMIZATION_REVIEW_PLAN_SCHEMA_VERSION {
            return Err(argus_core::ArgusError::unsupported(format!(
                "unsupported optimization review plan schema version {}",
                self.schema_version
            )));
        }
        if self.applicability.state != ApplicabilityState::Applicable {
            return Err(argus_core::ArgusError::invalid_input(
                "only applicable optimization review units can be materialized",
            ));
        }
        ...
        for id in &self.evidence {
            let hash = catalog.hashes.get(id).ok_or_else(|| {
                argus_core::ArgusError::invariant(
                    "optimization review unit references uncatalogued evidence",
                )
            })?;
            let stored = store.get(hash)?;
            if stored.envelope.record.id != *id {
                return Err(argus_core::ArgusError::invariant(
                    "optimization evidence catalog identity mismatch",
                ));
            }
        ...
        Ok(OptimizationReviewMaterialization {
            unit: self.clone(),
            package,
            context,
            contract,
...
impl OptimizationReviewPlan {
    pub fn materialize_admissible(
        ...
        budget: &EvidenceBudget,
        maximum_classification: DataClassification,
    ) -> Result<OptimizationReviewBatch, argus_core::ArgusError> {
        ...
        Ok(OptimizationReviewBatch { materializations })
    }
}

impl OptimizationReviewBatch {
    pub fn admit(
        &self,
        queue: &DurableQueue,
        ...
        if adapter.trim().is_empty() || adapter.trim() != adapter {
            return Err(argus_core::ArgusError::invalid_input(
                "optimization adapter identity must be normalized",
            ));
        }
        ...
        for materialized in &self.materializations {
            let package_bytes =
                serde_json::to_vec(&materialized.package.package).map_err(|error| {
                    argus_core::ArgusError::invariant(
                        "cannot serialize optimization evidence package",
                    )
                    .with_source(error)
                })?;
            ...
            if context.content_hash != materialized.context.hash {
                return Err(argus_core::ArgusError::invariant(
                    "stored optimization review context identity mismatch",
                ));
            }
            let admission = OptimizationReviewAdmission {
                schema_version: OPTIMIZATION_REVIEW_PLAN_SCHEMA_VERSION,
            ...
            let payload = serde_json::to_vec(&admission).map_err(|error| {
                argus_core::ArgusError::invariant(
                    "cannot serialize optimization review admission payload",
                )
                .with_source(error)
            })?;
...
pub struct OptimizationReviewPlanner<'a> {
    policy: &'a OptimizationApplicabilityPolicy,
    policy_id: PolicyId,
    policy_version: String,
}

impl<'a> OptimizationReviewPlanner<'a> {
    pub fn new(
        policy: &'a OptimizationApplicabilityPolicy,
        policy_id: PolicyId,
        ...
        if policy_version.trim().is_empty() || policy_version.trim() != policy_version {
            return Err(argus_core::ArgusError::invalid_input(
                "optimization policy version must be normalized",
            ));
        }
        Ok(Self {
            policy,
    ...
    pub fn plan(
    ...
    ) -> Result<OptimizationReviewPlan, argus_core::ArgusError> {
        ...
        if targets_by_id.len() != targets.len() {
            return Err(argus_core::ArgusError::invariant(
                "optimization plan contains duplicate targets",
            ));
        }
        ...
        for record in evidence {
            ...
            if let Some(target) = &record.target {
                if !targets_by_id.contains_key(target) {
                    return Err(argus_core::ArgusError::invariant(
                        "optimization evidence references an unknown target",
                    ));
                }
                evidence_by_target
                    .entry(target.clone())
        ...
        for target in targets_by_id.values() {
            let mut profile = OptimizationTargetProfile::from_target(target);
            ...
            let work_item = WorkItemId::derive([
                b"optimization-review".as_slice(),
            ...
            units.push(OptimizationReviewUnit {
                schema_version: OPTIMIZATION_REVIEW_PLAN_SCHEMA_VERSION,
                work_item,
                target: profile,
        ...
        Ok(OptimizationReviewPlan { units })
    }
}
...
fn target_class_label(class: OptimizationTargetClass) -> String {
...
fn validate_restored_identity(
    ...
    {
        return Err(argus_core::ArgusError::invariant(
            "restored optimization review identity mismatch",
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn planner_creates_work_items_for_executable_declarations() {
        let policy = OptimizationApplicabilityPolicy::conservative().unwrap();
        let planner = OptimizationReviewPlanner::new(
            &policy,
            PolicyId::derive([b"opt-policy".as_slice()]),
