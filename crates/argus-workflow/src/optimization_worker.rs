...
use crate::{
    DocumentationWorkerRuntime, OptimizationReviewAdmission, OptimizationReviewMaterialization,
    OptimizationRuntimeIdentity, RECOVERY_MANIFEST_SCHEMA_VERSION, RecoveryError, RecoveryManifest,
    RecoveryStore, WORKFLOW_DATA_SCHEMA_VERSION, WorkflowDataStore, open_checkpoint_store,
    optimization_actor_registry,
};
...
#[derive(Clone, Debug)]
pub struct OptimizationWorkerConfig {
    pub state_directory: PathBuf,
    pub identity: OptimizationRuntimeIdentity,
    pub adapter: String,
    pub policy: String,
...
pub struct OptimizationWorker {
    ...
    config: OptimizationWorkerConfig,
    checkpoint_store: Arc<dyn CheckpointStore>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum OptimizationWorkerResult {
    Idle,
...
struct PreparedOptimizationRuntime {
...
impl OptimizationWorker {
    pub fn new(
        ...
        if config.lease_duration_millis == 0 || config.maximum_attempts == 0 {
            return Err(ArgusError::invalid_input(
                "optimization worker lease and attempt limits must be positive",
            ));
        }
        if config.adapter.is_empty() || config.policy.is_empty() {
            return Err(ArgusError::invalid_input(
                "optimization worker adapter and policy must not be empty",
            ));
        }
    ...
    pub async fn run_next(&self, now_millis: u64) -> Result<OptimizationWorkerResult, ArgusError> {
        let leased = {
            ...
            .await
            .map_err(|error| {
                ArgusError::invariant("optimization lease task failed").with_source(error)
            })??
        };
        let Some(leased) = leased else {
            return Ok(OptimizationWorkerResult::Idle);
        };
            ...
            Err(error) => {
                ...
                let succeeded_already = {
                    ...
                    tokio::task::spawn_blocking(move || queue.get(&work_id))
                        .await
                        .map_err(|error| {
                            ArgusError::invariant("optimization lookup task failed")
                                .with_source(error)
                        })??
                };
                if succeeded_already.is_some_and(|work| work.state == QueueState::Succeeded) {
                    return Ok(OptimizationWorkerResult::Succeeded { work_id: leased.id });
                }
                let queue = self.queue.clone();
                let work_id = leased.id.clone();
                ...
                Ok(match state {
                    ...
                    QueueState::Failed => OptimizationWorkerResult::Failed {
                        work_id: leased.id,
                        error: message,
                    },
                    _ => {
                        return Err(ArgusError::invariant(
                            "failed optimization attempt entered an invalid queue state",
                        ));
                    }
                })
            }
    ...
    async fn execute_with_heartbeats(
        ...
        let mut heartbeat_task = tokio::spawn(async move {
            ...
            loop {
                ...
                .await
                .map_err(|error| {
                    ArgusError::invariant("optimization heartbeat task failed").with_source(error)
                })??;
            }
        ...
        let result = tokio::select! {
            ...
            heartbeat = &mut heartbeat_task => heartbeat
                .map_err(|error| ArgusError::invariant("optimization heartbeat task failed").with_source(error))?,
        };
    ...
    async fn execute(&self, leased: &LeasedWork) -> Result<(), ArgusError> {
        ...
        tracing::info!(
            ...
            "Processing optimization review for {:?} target `{}`",
        ...
        let checkpoint = prepared
            ...
            .await
            .map_err(|error| {
                ArgusError::invariant("cannot load optimization checkpoint").with_source(error)
            })?;
        let resolver = Arc::new(OptimizationContextResolver {
            ...
            content: String::from_utf8(materialized.context.canonical_json.clone()).map_err(
                |error| {
                    ArgusError::invariant("optimization context is not UTF-8").with_source(error)
                },
            )?,
        ...
        if let Some(snapshot) = checkpoint {
            let checkpoint: InstanceCheckpoint = serde_json::from_slice(&snapshot.payload)
                .map_err(|error| {
                    ArgusError::invalid_input("invalid optimization workflow checkpoint")
                        .with_source(error)
                })?;
            match checkpoint.status {
                RunStatus::Suspended => {
                    ...
                    instance.resume().await.map_err(|error| {
                        ArgusError::invariant("cannot resume optimization workflow")
                            .with_source(error)
                    })?;
                }
                RunStatus::Completed => {
                ...
                RunStatus::Failed | RunStatus::Cancelled | RunStatus::Running => {
                    return Err(ArgusError::invariant(
                        "optimization checkpoint is not safely resumable",
                    ));
                }
            }
        } else {
            instance.start().await.map_err(|error| {
                ArgusError::invariant("cannot start optimization workflow").with_source(error)
            })?;
        }
        let status = instance.run_to_completion().await.map_err(|error| {
            ArgusError::invariant("optimization workflow execution failed").with_source(error)
        })?;
        if status != RunStatus::Completed {
            ...
            return Err(ArgusError::invariant(format!(
                "optimization workflow ended with {status:?}{detail}"
            )));
        }
    ...
    fn restore_work(
        ...
        (
            OptimizationReviewAdmission,
            OptimizationReviewMaterialization,
            RunId,
        ),
        ArgusError,
    > {
        ...
        if admission.unit.work_item != leased.id {
            return Err(ArgusError::invariant(
                "leased work does not match its optimization admission",
            ));
        }
        let owned = self
            .queue
            .get(&leased.id)?
            .ok_or_else(|| ArgusError::invariant("leased optimization work is missing"))?;
        if owned.run != self.config.identity.audit_run
            || owned.coverage.snapshot != self.config.identity.audit_snapshot.as_str()
        {
            return Err(ArgusError::invariant(
                "optimization worker identity does not own the leased work",
            ));
        }
        let materialized = OptimizationReviewMaterialization::restore(&self.queue, &admission)?;
        if materialized.package.package.snapshot != self.config.identity.audit_snapshot {
            return Err(ArgusError::invariant(
                "optimization package is outside the worker snapshot",
            ));
        }
        ...
        let retry_generation = u64::try_from(retry_generation)
            .map_err(|_| ArgusError::invariant("optimization retry generation overflow"))?;
        ...
        materialized
            ...
            .map_err(|error| {
                ArgusError::invariant("cannot initialize optimization workflow data")
                    .with_source(error)
            })?;
    ...
    fn prepare_runtime(
        &self,
        leased: &LeasedWork,
        materialized: &OptimizationReviewMaterialization,
        langchart_run_id: &RunId,
    ) -> Result<PreparedOptimizationRuntime, ArgusError> {
        let recovery = RecoveryStore::open(&self.config.state_directory).map_err(|error| {
            ArgusError::invariant("cannot open optimization recovery store").with_source(error)
        })?;
        let workflow = recovery.store_target_review().map_err(|error| {
            ArgusError::invariant("cannot store optimization workflow").with_source(error)
        })?;
        let manifest = match recovery.load_manifest(langchart_run_id.as_ref()) {
            Ok(mut existing) => {
                ...
                existing.actors =
                    recovery
                        ...
                        .map_err(|error| {
                            ArgusError::invariant("cannot resolve optimization actor identities")
                                .with_source(error)
                        })?;
                existing
            }
            Err(RecoveryError::Io(error)) if error.kind() == std::io::ErrorKind::NotFound => {
                ...
                recovery.write_manifest(&manifest).map_err(|error| {
                    ArgusError::invariant("cannot store optimization recovery manifest")
                        .with_source(error)
                })?;
                manifest
            }
            Err(error) => {
                return Err(
                    ArgusError::invariant("cannot load optimization recovery manifest")
                        .with_source(error),
                );
            }
        };
        let compiled = Arc::new(
            recovery
                ...
                .map_err(|error| {
                    ArgusError::invariant("cannot load optimization workflow").with_source(error)
                })?,
        );
        let registry = optimization_actor_registry(
            self.queue.clone(),
        ...
        .map_err(|error| {
            ArgusError::invariant("cannot assemble optimization actor registry").with_source(error)
        })?;
        let actors = registry.reconstruct(&manifest).map_err(|error| {
            ArgusError::invariant("cannot reconstruct optimization actors").with_source(error)
        })?;
        Ok(PreparedOptimizationRuntime {
            compiled,
            actors,
    ...
    fn require_durable_outcome(&self, leased: &LeasedWork) -> Result<(), ArgusError> {
        let work = self
            .queue
            .get(&leased.id)?
            .ok_or_else(|| ArgusError::invariant("optimization work disappeared"))?;
        if work.state != QueueState::Succeeded {
            return Err(ArgusError::invariant(
                "optimization workflow completed without a durable outcome",
            ));
        }
        Ok(())
    }
    ...
    fn require_durable_result(
        ...
        let record = self
            ...
            .load(langchart_run_id)
            .map_err(|error| {
                ArgusError::invariant("cannot load terminal optimization decision")
                    .with_source(error)
            })?
            .ok_or_else(|| ArgusError::invariant("terminal optimization decision is missing"))?;
            ...
            && decision.event_type == "review.failed"
        {
            ...
            return Err(ArgusError::invariant(format!(
                "optimization review declared failure: {reason}"
            )));
        }
...
fn langchart_run_id(audit_run: &AuditRunId, work_id: &WorkItemId, retry_generation: u64) -> RunId {
    ...
    hasher.update(b"argus.optimization.langchart-run.v1\0");
    ...
    RunId::new(format!("argus-optimization-{}", hasher.finalize().to_hex()))
}

struct OptimizationContextResolver {
    run_id: RunId,
    source: String,
    content: String,
    tokens: u32,
...
#[async_trait]
impl ContextResolver for OptimizationContextResolver {
    async fn resolve(
        &self,
