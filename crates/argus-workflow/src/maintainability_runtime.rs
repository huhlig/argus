...
use crate::{
    ActorRegistry, ActorRegistryError, CandidateRecorderActor, DurableMaintainabilityOutcomeActor,
    EvidenceRequestEvaluatorActor, FindingWorkSchedulerActor, MaintainabilityReviewMaterialization,
...
/// Runtime identity and configuration parameters for maintainability review execution.
#[derive(Clone, Debug)]
pub struct MaintainabilityRuntimeIdentity {
...
/// Builds and populates an [`ActorRegistry`] with the actors required for maintainability review.
pub fn maintainability_actor_registry(
    queue: Arc<DurableQueue>,
    workflow_data: Arc<WorkflowDataStore>,
    materialized: &MaintainabilityReviewMaterialization,
    executor: Arc<ProviderExecutor>,
    identity: MaintainabilityRuntimeIdentity,
) -> Result<ActorRegistry, ActorRegistryError> {
    let mut registry = ActorRegistry::new();
    register(
        &mut registry,
        "argus.prepare-evidence",
        Arc::new(PrepareMaintainabilityEvidenceActor::new(
            workflow_data.clone(),
            materialized.clone(),
        )),
    )?;
    ...
    register(
        ...
        Arc::new(DurableMaintainabilityOutcomeActor::new(
            queue,
            workflow_data,
...
struct PrepareMaintainabilityEvidenceActor {
    workflow_data: Arc<WorkflowDataStore>,
    materialized: MaintainabilityReviewMaterialization,
}

impl PrepareMaintainabilityEvidenceActor {
    const fn new(
        workflow_data: Arc<WorkflowDataStore>,
        materialized: MaintainabilityReviewMaterialization,
    ) -> Self {
...
#[async_trait]
impl AgentActor for PrepareMaintainabilityEvidenceActor {
    async fn run(
        &self,
        ...
        tracing::debug!(
            actor = "PrepareMaintainabilityEvidenceActor",
            ...
            "Entering workflow state: PrepareMaintainabilityEvidenceActor"
        );
            ...
            || record.data.evidence_revision != self.materialized.package.package.revision
        {
            return Err(AgentError::Internal(
                "prepared maintainability evidence identity mismatch".to_owned(),
            ));
        }
        tracing::debug!(
            actor = "PrepareMaintainabilityEvidenceActor",
            "Exiting workflow state: PrepareMaintainabilityEvidenceActor -> evidence.prepared"
        );
        Ok(AgentOutputEvent {
...
impl AgentActor for DisabledEvidenceExpansionActor {
    async fn run(
        ...
        Err(AgentError::Internal(
            "maintainability evidence expansion is disabled for this admission".to_owned(),
        ))
    }
}
