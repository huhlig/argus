...
/// Runtime identity and configuration parameters for documentation review execution.
#[derive(Clone, Debug)]
...
/// Builds and populates an [`ActorRegistry`] with the actors required for documentation review.
///
/// Registers actors for the standard documentation review pipeline:
/// - `argus.prepare-evidence`: Prepares documentation evidence from materialized targets.
...
impl AgentActor for PrepareDocumentationEvidenceActor {
    async fn run(
        ...
        tracing::debug!(
            actor = "PrepareDocumentationEvidenceActor",
            ...
            "Entering workflow state: PrepareDocumentationEvidenceActor"
        );
        ...
        tracing::debug!(
            actor = "PrepareDocumentationEvidenceActor",
            "Exiting workflow state: PrepareDocumentationEvidenceActor -> evidence.prepared"
        );
        Ok(AgentOutputEvent {
