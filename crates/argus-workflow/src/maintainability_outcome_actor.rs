...
use crate::{
    EffectiveOutcome, LogicalOutcomeKey, MaintainabilityAssessmentContract, OutcomeDisposition,
...
use argus_policies::{MaintainabilityAssessment, MaintainabilityResult};
...
/// Stores one validated maintainability assessment before committing its durable outcome reference.
pub struct MaintainabilityOutcomeActor {
    inbox: Arc<DurableQueue>,
    assessment: MaintainabilityAssessment,
    logical_key: LogicalOutcomeKey,
    provenance: OutcomeProvenance,
}

pub struct DurableMaintainabilityOutcomeActor {
    inbox: Arc<DurableQueue>,
    workflow_data: Arc<WorkflowDataStore>,
    contract: Arc<MaintainabilityAssessmentContract>,
...
impl DurableMaintainabilityOutcomeActor {
    ...
    async fn record_for_run(&self, langchart_run_id: &str) -> Result<OutcomeReceipt, String> {
        ...
        MaintainabilityOutcomeActor::from_decision(
            self.inbox.clone(),
...
impl MaintainabilityOutcomeActor {
    pub fn from_decision(
        inbox: Arc<DurableQueue>,
        contract: &MaintainabilityAssessmentContract,
    ...
    pub fn new(
            ...
            || assessment.policy_version != logical_key.policy_version
        {
            return Err(argus_core::ArgusError::invariant(
                "maintainability assessment identity does not match its outcome key",
            ));
        }
        Ok(Self {
            inbox,
    ...
    pub fn record(&self) -> Result<OutcomeReceipt, String> {
        ...
        let stored = self
            .inbox
            .store_artifact(MAINTAINABILITY_ASSESSMENT_ARTIFACT_KIND, &assessment_bytes)
            .map_err(|error| error.to_string())?;
        let effective_outcome = EffectiveOutcome {
            logical_key: self.logical_key.clone(),
            result_ref: stored.reference,
            kind: match self.assessment.result {
                MaintainabilityResult::Passed => OutcomeKind::Passed,
                MaintainabilityResult::CandidateFindings { .. } => OutcomeKind::CandidateFindings,
                MaintainabilityResult::UnableToVerify { .. } => OutcomeKind::UnableToVerify,
            },
...
#[async_trait]
impl AgentActor for DurableMaintainabilityOutcomeActor {
    async fn run(
        &self,
