...
use argus_policies::{
    MaintainabilityApplicability, MaintainabilityApplicabilityPolicy, MaintainabilityTargetClass,
...
pub const MAINTAINABILITY_REVIEW_PLAN_SCHEMA_VERSION: u32 = 1;
pub const MAINTAINABILITY_EVIDENCE_PACKAGE_ARTIFACT_KIND: &str = "evidence-package";
pub const MAINTAINABILITY_REVIEW_CONTEXT_ARTIFACT_KIND: &str = "review-context";

#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct MaintainabilityReviewUnit {
    pub schema_version: u32,
    pub work_item: WorkItemId,
    pub target: MaintainabilityTargetProfile,
    pub policy: PolicyId,
    pub policy_version: String,
    pub applicability: MaintainabilityApplicability,
    pub evidence: Vec<EvidenceId>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct MaintainabilityReviewPlan {
    pub units: Vec<MaintainabilityReviewUnit>,
}

#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct MaintainabilityReviewAdmission {
    pub schema_version: u32,
    pub unit: MaintainabilityReviewUnit,
    pub evidence_package_ref: String,
    pub review_context_ref: String,
}

#[derive(Clone, Debug)]
pub struct MaintainabilityReviewBatch {
    pub materializations: Vec<MaintainabilityReviewMaterialization>,
}

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct MaintainabilityEvidenceCatalog {
    hashes: BTreeMap<EvidenceId, ContentHash>,
}

impl MaintainabilityEvidenceCatalog {
    pub fn ingest(
        store: &EvidenceStore,
        snapshot: &SnapshotId,
        ...
        for record in records {
            if hashes.contains_key(&record.id) {
                return Err(argus_core::ArgusError::invariant(
                    "maintainability evidence catalog contains duplicate IDs",
                ));
            }
            let envelope =
...
#[derive(Clone, Debug)]
pub struct MaintainabilityReviewMaterialization {
    pub unit: MaintainabilityReviewUnit,
    pub package: PackageArtifact,
    pub context: ContextArtifact,
    pub contract: Arc<crate::MaintainabilityAssessmentContract>,
}

impl MaintainabilityReviewMaterialization {
    pub fn restore(
        queue: &DurableQueue,
        admission: &MaintainabilityReviewAdmission,
    ) -> Result<Self, argus_core::ArgusError> {
        if admission.schema_version != MAINTAINABILITY_REVIEW_PLAN_SCHEMA_VERSION
            || admission.unit.schema_version != MAINTAINABILITY_REVIEW_PLAN_SCHEMA_VERSION
            || admission.unit.applicability.state != ApplicabilityState::Applicable
        {
            return Err(argus_core::ArgusError::unsupported(
                "unsupported or inapplicable maintainability review admission",
            ));
        }
        let stored_package = load_artifact(
            queue,
            &admission.evidence_package_ref,
            MAINTAINABILITY_EVIDENCE_PACKAGE_ARTIFACT_KIND,
        )?;
        ...
        let stored_context = load_artifact(
            ...
            MAINTAINABILITY_REVIEW_CONTEXT_ARTIFACT_KIND,
        )?;
        ...
        let contract = Arc::new(crate::MaintainabilityAssessmentContract::from_context(
...
impl MaintainabilityReviewUnit {
    pub fn materialize(
        &self,
        store: &EvidenceStore,
        catalog: &MaintainabilityEvidenceCatalog,
        snapshot: &SnapshotId,
        configuration: &ConfigurationId,
        budget: EvidenceBudget,
        maximum_classification: DataClassification,
    ) -> Result<MaintainabilityReviewMaterialization, argus_core::ArgusError> {
        if self.schema_version != MAINTAINABILITY_REVIEW_PLAN_SCHEMA_VERSION {
            return Err(argus_core::ArgusError::unsupported(format!(
                "unsupported maintainability review plan schema version {}",
                self.schema_version
            )));
        }
        if self.applicability.state != ApplicabilityState::Applicable {
            return Err(argus_core::ArgusError::invalid_input(
                "only applicable maintainability review units can be materialized",
            ));
        }
        ...
        for id in &self.evidence {
            let hash = catalog.hashes.get(id).ok_or_else(|| {
                argus_core::ArgusError::invariant(
                    "maintainability review unit references uncatalogued evidence",
                )
            })?;
            let stored = store.get(hash)?;
            if stored.envelope.record.id != *id {
                return Err(argus_core::ArgusError::invariant(
                    "maintainability evidence catalog identity mismatch",
                ));
            }
        ...
        Ok(MaintainabilityReviewMaterialization {
            unit: self.clone(),
            package,
            context,
            contract,
...
impl MaintainabilityReviewPlan {
    pub fn materialize_admissible(
        ...
        budget: &EvidenceBudget,
        maximum_classification: DataClassification,
    ) -> Result<MaintainabilityReviewBatch, argus_core::ArgusError> {
        ...
        Ok(MaintainabilityReviewBatch { materializations })
    }
}

impl MaintainabilityReviewBatch {
    pub fn admit(
        &self,
        queue: &DurableQueue,
        ...
        if adapter.trim().is_empty() || adapter.trim() != adapter {
            return Err(argus_core::ArgusError::invalid_input(
                "maintainability adapter identity must be normalized",
            ));
        }
        ...
        for materialized in &self.materializations {
            let package_bytes =
                serde_json::to_vec(&materialized.package.package).map_err(|error| {
                    argus_core::ArgusError::invariant(
                        "cannot serialize maintainability evidence package",
                    )
                    .with_source(error)
                })?;
            ...
            if package.content_hash != materialized.package.hash {
                return Err(argus_core::ArgusError::invariant(
                    "stored maintainability evidence package identity mismatch",
                ));
            }
            ...
            if context.content_hash != materialized.context.hash {
                return Err(argus_core::ArgusError::invariant(
                    "stored maintainability review context identity mismatch",
                ));
            }
            let admission = MaintainabilityReviewAdmission {
                schema_version: MAINTAINABILITY_REVIEW_PLAN_SCHEMA_VERSION,
            ...
            let payload = serde_json::to_vec(&admission).map_err(|error| {
                argus_core::ArgusError::invariant(
                    "cannot serialize maintainability review admission payload",
                )
                .with_source(error)
            })?;
...
pub struct MaintainabilityReviewPlanner<'a> {
    policy: &'a MaintainabilityApplicabilityPolicy,
    policy_id: PolicyId,
    policy_version: String,
}

impl<'a> MaintainabilityReviewPlanner<'a> {
    pub fn new(
        policy: &'a MaintainabilityApplicabilityPolicy,
        policy_id: PolicyId,
        ...
        if policy_version.trim().is_empty() || policy_version.trim() != policy_version {
            return Err(argus_core::ArgusError::invalid_input(
                "maintainability policy version must be normalized",
            ));
        }
        Ok(Self {
            policy,
    ...
    pub fn plan(
    ...
    ) -> Result<MaintainabilityReviewPlan, argus_core::ArgusError> {
        ...
        if targets_by_id.len() != targets.len() {
            return Err(argus_core::ArgusError::invariant(
                "maintainability plan contains duplicate targets",
            ));
        }
        ...
        for record in evidence {
            ...
            if let Some(target) = &record.target {
                if !targets_by_id.contains_key(target) {
                    return Err(argus_core::ArgusError::invariant(
                        "maintainability evidence references an unknown target",
                    ));
                }
                evidence_by_target
                    .entry(target.clone())
        ...
        for target in targets_by_id.values() {
            let mut profile = MaintainabilityTargetProfile::from_target(target);
            ...
            let work_item = WorkItemId::derive([
                b"maintainability-review".as_slice(),
            ...
            units.push(MaintainabilityReviewUnit {
                schema_version: MAINTAINABILITY_REVIEW_PLAN_SCHEMA_VERSION,
                work_item,
                target: profile,
        ...
        Ok(MaintainabilityReviewPlan { units })
    }
}
...
fn target_class_label(class: MaintainabilityTargetClass) -> String {
...
fn validate_restored_identity(
    unit: &MaintainabilityReviewUnit,
    ...
    {
        return Err(argus_core::ArgusError::invariant(
            "restored maintainability review identity mismatch",
        ));
    }
    Ok(())
}
...
mod tests {
    use super::*;
    use argus_core::{PortableTargetKind, TargetId, TargetKind, TargetVisibility};

    #[test]
    fn planner_creates_work_items_for_maintainability_targets() {
        let policy = MaintainabilityApplicabilityPolicy::conservative().unwrap();
        let planner = MaintainabilityReviewPlanner::new(
            &policy,
            PolicyId::derive([b"maint-policy".as_slice()]),
            "maintainability-conservative@1",
        )
        .unwrap();
        let snapshot = SnapshotId::derive([b"snap".as_slice()]);
        let config = ConfigurationId::derive([b"cfg".as_slice()]);
        let target = Target {
            id: TargetId::derive([b"fn-test".as_slice()]),
