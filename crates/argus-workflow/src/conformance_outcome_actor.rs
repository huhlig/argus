...
use argus_policies::{ConformanceAssessment, ConformanceResult};
...
/// Stores one validated conformance assessment before committing its durable outcome reference.
pub struct ConformanceOutcomeActor {
...
pub struct DurableConformanceOutcomeActor {
    inbox: Arc<DurableQueue>,
    workflow_data: Arc<WorkflowDataStore>,
    contract: Arc<ConformanceAssessmentContract>,
...
impl DurableConformanceOutcomeActor {
    ...
    async fn record_for_run(&self, langchart_run_id: &str) -> Result<OutcomeReceipt, String> {
        ...
        ConformanceOutcomeActor::from_decision(
            self.inbox.clone(),
...
impl ConformanceOutcomeActor {
    pub fn from_decision(
        inbox: Arc<DurableQueue>,
        contract: &ConformanceAssessmentContract,
    ...
    pub fn new(
            ...
            || assessment.policy_version != logical_key.policy_version
        {
            return Err(argus_core::ArgusError::invariant(
                "conformance assessment identity does not match its outcome key",
            ));
        }
        Ok(Self {
            inbox,
    ...
    pub fn record(&self) -> Result<OutcomeReceipt, String> {
        ...
        let stored = self
            .inbox
            .store_artifact(CONFORMANCE_ASSESSMENT_ARTIFACT_KIND, &assessment_bytes)
            .map_err(|error| error.to_string())?;
        let effective_outcome = EffectiveOutcome {
            ...
            kind: match self.assessment.result {
                ConformanceResult::Passed => OutcomeKind::Passed,
                ConformanceResult::CandidateFindings { .. } => OutcomeKind::CandidateFindings,
                ConformanceResult::UnableToVerify { .. } => OutcomeKind::UnableToVerify,
            },
...
#[async_trait]
impl AgentActor for DurableConformanceOutcomeActor {
    async fn run(
        &self,
