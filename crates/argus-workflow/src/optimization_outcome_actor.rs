...
use crate::{
    EffectiveOutcome, LogicalOutcomeKey, OptimizationAssessmentContract, OutcomeDisposition,
...
use argus_policies::{OptimizationAssessment, OptimizationResult};
...
/// Stores one validated optimization assessment before committing its durable outcome reference.
pub struct OptimizationOutcomeActor {
...
pub struct DurableOptimizationOutcomeActor {
    inbox: Arc<DurableQueue>,
    workflow_data: Arc<WorkflowDataStore>,
    contract: Arc<OptimizationAssessmentContract>,
...
impl DurableOptimizationOutcomeActor {
    ...
    async fn record_for_run(&self, langchart_run_id: &str) -> Result<OutcomeReceipt, String> {
        ...
        OptimizationOutcomeActor::from_decision(
            self.inbox.clone(),
...
impl OptimizationOutcomeActor {
    pub fn from_decision(
        inbox: Arc<DurableQueue>,
        contract: &OptimizationAssessmentContract,
    ...
    pub fn new(
            ...
            || assessment.policy_version != logical_key.policy_version
        {
            return Err(argus_core::ArgusError::invariant(
                "optimization assessment identity does not match its outcome key",
            ));
        }
        Ok(Self {
            inbox,
    ...
    pub fn record(&self) -> Result<OutcomeReceipt, String> {
        ...
        let stored = self
            .inbox
            .store_artifact(OPTIMIZATION_ASSESSMENT_ARTIFACT_KIND, &assessment_bytes)
            .map_err(|error| error.to_string())?;
        let effective_outcome = EffectiveOutcome {
            ...
            kind: match self.assessment.result {
                OptimizationResult::Passed => OutcomeKind::Passed,
                OptimizationResult::CandidateFindings { .. } => OutcomeKind::CandidateFindings,
                OptimizationResult::UnableToVerify { .. } => OutcomeKind::UnableToVerify,
            },
...
#[async_trait]
impl AgentActor for DurableOptimizationOutcomeActor {
    async fn run(
        &self,
