...
#[derive(Clone, Debug)]
pub struct CorrectnessWorkerConfig {
    pub state_directory: PathBuf,
    pub identity: CorrectnessRuntimeIdentity,
    pub adapter: String,
    pub policy: String,
...
pub enum CorrectnessWorkerResult {
    Idle,
...
impl CorrectnessWorker {
    pub fn new(
        ...
        if config.lease_duration_millis == 0 || config.maximum_attempts == 0 {
            return Err(ArgusError::invalid_input(
                "correctness worker lease and attempt limits must be positive",
            ));
        }
        ...
        match self.execute_with_heartbeats(&leased, now_millis).await {
            ...
            Err(error) => {
                ...
                Ok(match state {
                    ...
                    QueueState::Failed => CorrectnessWorkerResult::Failed {
                        work_id: leased.id,
                        error: message,
                    },
                    _ => {
                        return Err(ArgusError::invariant(
                            "failed correctness attempt entered an invalid queue state",
                        ));
                    }
                })
            }
    ...
    async fn execute_with_heartbeats(
        ...
        let result = tokio::select! {
            ...
            heartbeat = &mut heartbeat_task => heartbeat
                .map_err(|error| ArgusError::invariant("correctness heartbeat task failed").with_source(error))?,
        };
    ...
    async fn execute(&self, leased: &LeasedWork) -> Result<(), ArgusError> {
        ...
        let checkpoint = prepared
            ...
            .await
            .map_err(|error| {
                ArgusError::invariant("cannot load correctness checkpoint").with_source(error)
            })?;
        let resolver = Arc::new(CorrectnessContextResolver {
            ...
            content: String::from_utf8(materialized.context.canonical_json.clone()).map_err(
                |error| {
                    ArgusError::invariant("correctness context is not UTF-8").with_source(error)
                },
            )?,
        ...
        if let Some(snapshot) = checkpoint {
            let checkpoint: InstanceCheckpoint = serde_json::from_slice(&snapshot.payload)
                .map_err(|error| {
                    ArgusError::invalid_input("invalid correctness workflow checkpoint")
                        .with_source(error)
                })?;
            match checkpoint.status {
                RunStatus::Suspended => {
                    ...
                    instance.resume().await.map_err(|error| {
                        ArgusError::invariant("cannot resume correctness workflow")
                            .with_source(error)
                    })?;
                }
                RunStatus::Completed => {
                ...
                RunStatus::Failed | RunStatus::Cancelled | RunStatus::Running => {
                    return Err(ArgusError::invariant(
                        "correctness checkpoint is not safely resumable",
                    ));
                }
            }
        } else {
            instance.start().await.map_err(|error| {
                ArgusError::invariant("cannot start correctness workflow").with_source(error)
            })?;
        }
    ...
    fn restore_work(
        ...
        (
            CorrectnessReviewAdmission,
            CorrectnessReviewMaterialization,
            RunId,
        ),
        ArgusError,
    > {
        ...
        if admission.unit.work_item != leased.id {
            return Err(ArgusError::invariant(
                "leased work does not match its correctness admission",
            ));
        }
        let owned = self
            .queue
            .get(&leased.id)?
            .ok_or_else(|| ArgusError::invariant("leased correctness work is missing"))?;
        if owned.run != self.config.identity.audit_run
            || owned.coverage.snapshot != self.config.identity.audit_snapshot.as_str()
        {
            return Err(ArgusError::invariant(
                "correctness worker identity does not own the leased work",
            ));
        }
        let materialized = CorrectnessReviewMaterialization::restore(&self.queue, &admission)?;
        if materialized.package.package.snapshot != self.config.identity.audit_snapshot {
            return Err(ArgusError::invariant(
                "correctness package is outside the worker snapshot",
            ));
        }
        ...
        let retry_generation = u64::try_from(retry_generation)
            .map_err(|_| ArgusError::invariant("correctness retry generation overflow"))?;
        ...
        materialized
            ...
            .map_err(|error| {
                ArgusError::invariant("cannot initialize correctness workflow data")
                    .with_source(error)
            })?;
    ...
    fn prepare_runtime(
        &self,
        leased: &LeasedWork,
        materialized: &CorrectnessReviewMaterialization,
        langchart_run_id: &RunId,
    ) -> Result<PreparedCorrectnessRuntime, ArgusError> {
        let recovery = RecoveryStore::open(&self.config.state_directory).map_err(|error| {
            ArgusError::invariant("cannot open correctness recovery store").with_source(error)
        })?;
        ...
        let compiled = Arc::new(
            recovery
                ...
                .map_err(|error| {
                    ArgusError::invariant("cannot load correctness workflow").with_source(error)
                })?,
        );
        let registry = correctness_actor_registry(
            self.queue.clone(),
        ...
        .map_err(|error| {
            ArgusError::invariant("cannot assemble correctness actor registry").with_source(error)
        })?;
    ...
    fn require_durable_outcome(&self, leased: &LeasedWork) -> Result<(), ArgusError> {
        let work = self
            .queue
            .get(&leased.id)?
            .ok_or_else(|| ArgusError::invariant("correctness work disappeared"))?;
        if work.state != QueueState::Succeeded {
            return Err(ArgusError::invariant(
                "correctness workflow completed without a durable outcome",
            ));
        }
        Ok(())
    }
    ...
    fn require_durable_result(
        ...
        let record = self
            ...
            .load(langchart_run_id)
            .map_err(|error| {
                ArgusError::invariant("cannot load terminal correctness decision")
                    .with_source(error)
            })?
            .ok_or_else(|| ArgusError::invariant("terminal correctness decision is missing"))?;
            ...
            && decision.event_type == "review.failed"
        {
            ...
            return Err(ArgusError::invariant(format!(
                "correctness review declared failure: {reason}"
            )));
        }
...
fn langchart_run_id(audit_run: &AuditRunId, work_id: &WorkItemId, retry_generation: u64) -> RunId {
    ...
    hasher.update(b"argus.correctness.langchart-run.v1\0");
...
#[async_trait]
impl ContextResolver for CorrectnessContextResolver {
    async fn resolve(
        &self,
