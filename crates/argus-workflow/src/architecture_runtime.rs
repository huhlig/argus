...
pub fn architecture_actor_registry(
    ...
    identity: ArchitectureRuntimeIdentity,
) -> Result<ActorRegistry, ActorRegistryError> {
    let mut registry = ActorRegistry::new();
    register(
        &mut registry,
        "argus.prepare-evidence",
        Arc::new(PrepareArchitectureEvidenceActor::new(
            workflow_data.clone(),
            materialized.clone(),
        )),
    )?;
    ...
    register(
        ...
        Arc::new(DurableArchitectureOutcomeActor::new(
            queue,
            workflow_data,
...
struct PrepareArchitectureEvidenceActor {
    workflow_data: Arc<WorkflowDataStore>,
    materialized: ArchitectureReviewMaterialization,
}

impl PrepareArchitectureEvidenceActor {
    const fn new(
        workflow_data: Arc<WorkflowDataStore>,
        materialized: ArchitectureReviewMaterialization,
    ) -> Self {
...
#[async_trait]
impl AgentActor for PrepareArchitectureEvidenceActor {
    async fn run(
        &self,
