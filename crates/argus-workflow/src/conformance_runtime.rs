...
/// Runtime identity and configuration parameters for conformance review execution.
#[derive(Clone, Debug)]
pub struct ConformanceRuntimeIdentity {
...
pub fn conformance_actor_registry(
    ...
    register(
        ...
        Arc::new(DurableConformanceOutcomeActor::new(
            queue,
            workflow_data,
...
struct PrepareConformanceEvidenceActor {
    workflow_data: Arc<WorkflowDataStore>,
    materialized: ConformanceReviewMaterialization,
}

impl PrepareConformanceEvidenceActor {
    const fn new(
        workflow_data: Arc<WorkflowDataStore>,
        materialized: ConformanceReviewMaterialization,
    ) -> Self {
...
#[async_trait]
impl AgentActor for PrepareConformanceEvidenceActor {
    async fn run(
        &self,
        ...
        tracing::debug!(
            actor = "PrepareConformanceEvidenceActor",
            ...
            "Entering workflow state: PrepareConformanceEvidenceActor"
        );
            ...
            || record.data.evidence_revision != self.materialized.package.package.revision
        {
            return Err(AgentError::Internal(
                "prepared conformance evidence identity mismatch".to_owned(),
            ));
        }
        tracing::debug!(
            actor = "PrepareConformanceEvidenceActor",
            "Exiting workflow state: PrepareConformanceEvidenceActor -> evidence.prepared"
        );
        Ok(AgentOutputEvent {
...
impl AgentActor for DisabledEvidenceExpansionActor {
    async fn run(
        ...
        Err(AgentError::Internal(
            "conformance evidence expansion is disabled for this admission".to_owned(),
        ))
    }
}
