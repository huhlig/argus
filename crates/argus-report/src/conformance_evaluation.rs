...
pub const CONFORMANCE_CORPUS_SCHEMA_VERSION: u32 = 1;
pub const CONFORMANCE_EVALUATION_SCHEMA_VERSION: u32 = 1;

#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct ExpectedConformanceIssue {
    pub id: String,
    pub target: TargetId,
...
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct ConformanceEvaluationCorpus {
    pub schema_version: u32,
    pub name: String,
    pub version: String,
    pub policy_version: String,
    pub expected_issues: Vec<ExpectedConformanceIssue>,
    pub known_clean_targets: Vec<TargetId>,
}

#[derive(Clone, Debug, Default, Eq, PartialEq, Serialize, Deserialize)]
pub struct ConformanceEvaluationThresholds {
...
impl ConformanceEvaluation {
    pub fn to_json(&self) -> Result<Vec<u8>, argus_core::ArgusError> {
        serde_json::to_vec_pretty(self).map_err(|error| {
            argus_core::ArgusError::invariant("cannot serialize conformance evaluation")
                .with_source(error)
        })
    }
    ...
    pub fn check_thresholds(
        &self,
        thresholds: &ConformanceEvaluationThresholds,
    ) -> Result<(), Vec<String>> {
...
pub fn evaluate_conformance(
    corpus: &ConformanceEvaluationCorpus,
    reports: &[ConformanceReport],
    adjudications: &[HumanAdjudication],
) -> Result<ConformanceEvaluation, argus_core::ArgusError> {
    if reports.is_empty() {
        return Err(argus_core::ArgusError::invalid_input(
            "at least one report is required for conformance evaluation",
        ));
    }
...
mod tests {
    ...
    #[test]
    fn evaluate_empty_conformance_corpus() {
        let corpus = ConformanceEvaluationCorpus {
            schema_version: CONFORMANCE_CORPUS_SCHEMA_VERSION,
            name: "test-corpus".to_owned(),
            version: "1.0.0".to_owned(),
            policy_version: "conformance-design-aligned@1".to_owned(),
        ...
        assert_eq!(eval.recall.denominator, 0);
