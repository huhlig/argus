...
use crate::{EvaluationRate, OptimizationReport};
use argus_core::{AdjudicationState, HumanAdjudication, TargetId};
use argus_policies::OptimizationDimension;
use serde::{Deserialize, Serialize};
use std::collections::{BTreeMap, BTreeSet};

pub const OPTIMIZATION_CORPUS_SCHEMA_VERSION: u32 = 1;
pub const OPTIMIZATION_EVALUATION_SCHEMA_VERSION: u32 = 1;

#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct ExpectedOptimizationIssue {
    pub id: String,
    pub target: TargetId,
...
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct OptimizationEvaluationCorpus {
    pub schema_version: u32,
    pub name: String,
    pub version: String,
    pub policy_version: String,
    pub expected_issues: Vec<ExpectedOptimizationIssue>,
    pub known_clean_targets: Vec<TargetId>,
}

#[derive(Clone, Debug, Default, Eq, PartialEq, Serialize, Deserialize)]
pub struct OptimizationEvaluationThresholds {
...
impl OptimizationEvaluation {
    pub fn to_json(&self) -> Result<Vec<u8>, argus_core::ArgusError> {
        serde_json::to_vec_pretty(self).map_err(|error| {
            argus_core::ArgusError::invariant("cannot serialize optimization evaluation")
                .with_source(error)
        })
    }
    ...
    pub fn check_thresholds(
        &self,
        thresholds: &OptimizationEvaluationThresholds,
    ) -> Result<(), Vec<String>> {
...
impl OptimizationEvaluationCorpus {
    pub fn validate(&self) -> Result<(), argus_core::ArgusError> {
        if self.schema_version != OPTIMIZATION_CORPUS_SCHEMA_VERSION {
            return Err(argus_core::ArgusError::unsupported(format!(
                "unsupported optimization corpus schema version {}",
                self.schema_version
            )));
        }
            ...
            || self.expected_issues.is_empty()
        {
            return Err(argus_core::ArgusError::invalid_input(
                "optimization corpus identity and expected issues are required",
            ));
        }
        ...
        for issue in &self.expected_issues {
                ...
                || clean.contains(&issue.target)
            {
                return Err(argus_core::ArgusError::invariant(
                    "optimization corpus ground truth is ambiguous",
                ));
            }
        }
        if clean.len() != self.known_clean_targets.len() {
            return Err(argus_core::ArgusError::invariant(
                "optimization corpus repeats a known-clean target",
            ));
        }
        Ok(())
    }
...
pub fn evaluate_optimization(
    corpus: &OptimizationEvaluationCorpus,
    reports: &[OptimizationReport],
    adjudications: &[HumanAdjudication],
) -> Result<OptimizationEvaluation, argus_core::ArgusError> {
    corpus.validate()?;
    if reports.is_empty() {
        return Err(argus_core::ArgusError::invalid_input(
            "optimization evaluation requires at least one run",
        ));
    }
    ...
    {
        return Err(argus_core::ArgusError::invalid_input(
            "optimization evaluation reports must be distinct and match the corpus policy",
        ));
    }
    ...
    Ok(OptimizationEvaluation {
        schema_version: OPTIMIZATION_EVALUATION_SCHEMA_VERSION,
