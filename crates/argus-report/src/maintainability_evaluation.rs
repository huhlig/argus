...
use crate::{EvaluationRate, MaintainabilityReport};
use argus_core::{AdjudicationState, HumanAdjudication, TargetId};
use argus_policies::MaintainabilityDimension;
use serde::{Deserialize, Serialize};
use std::collections::{BTreeMap, BTreeSet};

pub const MAINTAINABILITY_CORPUS_SCHEMA_VERSION: u32 = 1;
pub const MAINTAINABILITY_EVALUATION_SCHEMA_VERSION: u32 = 1;

#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct ExpectedMaintainabilityIssue {
    pub id: String,
    pub target: TargetId,
...
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct MaintainabilityEvaluationCorpus {
    pub schema_version: u32,
    pub name: String,
    pub version: String,
    pub policy_version: String,
    pub expected_issues: Vec<ExpectedMaintainabilityIssue>,
    pub known_clean_targets: Vec<TargetId>,
}

#[derive(Clone, Debug, Default, Eq, PartialEq, Serialize, Deserialize)]
pub struct MaintainabilityEvaluationThresholds {
...
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct MaintainabilityEvaluation {
    pub schema_version: u32,
...
impl MaintainabilityEvaluation {
    pub fn to_json(&self) -> Result<Vec<u8>, argus_core::ArgusError> {
        serde_json::to_vec_pretty(self).map_err(|error| {
            argus_core::ArgusError::invariant("cannot serialize maintainability evaluation")
                .with_source(error)
        })
    }
...
impl MaintainabilityEvaluationCorpus {
    pub fn validate(&self) -> Result<(), argus_core::ArgusError> {
        if self.schema_version != MAINTAINABILITY_CORPUS_SCHEMA_VERSION {
            return Err(argus_core::ArgusError::unsupported(format!(
                "unsupported maintainability corpus schema version {}",
                self.schema_version
            )));
        }
            ...
            || self.expected_issues.is_empty()
        {
            return Err(argus_core::ArgusError::invalid_input(
                "maintainability corpus identity and expected issues are required",
            ));
        }
        ...
        for issue in &self.expected_issues {
                ...
                || clean.contains(&issue.target)
            {
                return Err(argus_core::ArgusError::invariant(
                    "maintainability corpus ground truth is ambiguous",
                ));
            }
        }
        if clean.len() != self.known_clean_targets.len() {
            return Err(argus_core::ArgusError::invariant(
                "maintainability corpus repeats a known-clean target",
            ));
        }
        Ok(())
    }
...
pub fn evaluate_maintainability(
...
) -> Result<MaintainabilityEvaluation, argus_core::ArgusError> {
    corpus.validate()?;
    if reports.is_empty() {
        return Err(argus_core::ArgusError::invalid_input(
            "maintainability evaluation requires at least one run",
        ));
    }
    ...
    {
        return Err(argus_core::ArgusError::invalid_input(
            "maintainability evaluation reports must be distinct and match the corpus policy",
        ));
    }
    ...
    for report in reports {
        ...
        for cluster in &report.finding_clusters {
            total_finding_occurrences += cluster.occurrences.len();
            if cluster.occurrences.len() > 1 {
                duplicate_finding_occurrences += cluster.occurrences.len() - 1;
            }
    ...
    Ok(MaintainabilityEvaluation {
        schema_version: MAINTAINABILITY_EVALUATION_SCHEMA_VERSION,
